...
(defun pick-color ()
  (second (nth (random (length *color-list*)) *color-list*)))
...
(flet ((base10 (str)
	 ...
	 (if (every #'alpha-char-p color-name)
	     (second (or (assoc color-name *color-list* :test #'string-equal)
			 (error "Unknown color : ~A" color-name)))
	     color-name)))
